...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	private final String USERS_QUERY = "select 	email, password, activate from user where email=?";
	private final String ROLES_QUERY = "select 	u.email, r.role  from user u inner join user_role ur on (u.id = ur.user_id) inner join role r on (ur.role_id=r.role.id) where email u.email=? ";
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/").permitAll().antMatchers("/login").permitAll().antMatchers("/signup")
				...
				.disable().formLogin().loginPage("/loginPage").failureUrl("/login?error=true")
